...
//
// The sim thread calls get_write_buffer() → [write] → publish().
// The render thread calls begin_read() → [read] → end_read().
//
// The render thread always sees the *latest completed* sim frame — it never
// blocks, and it never reads a partially-written buffer. If the sim outpaces
// the renderer the intermediate frame is silently overwritten (fine for
// visualisation; not appropriate if every frame must be processed).
// ─────────────────────────────────────────────────────────────────────────────
template<typename T>
class TripleBuffer
{
